...
class _QuizListPageState extends State<QuizListPage> {
  ...
  void initState() {
    context
        ...
        .add(const UjianByKategoriEvent.getUjianKategori('Verbal'));
    super.initState();
  }
          ...
          BlocListener<UjianByKategoriBloc, UjianByKategoriState>(
            listener: (context, state) {
              state.maybeWhen(
                orElse: () {},
                notfound: () {
                  //create ujiancontext.create
                  context
                      .read<CreateUjianBloc>()
                      .add(const CreateUjianEvent.createdUjian());
                },
              );
            },
            child: BlocBuilder<UjianByKategoriBloc, UjianByKategoriState>(
              builder: (context, state) {
                return state.maybeWhen(
                  ...
                  notfound: () {
                    return Center(
                      child: BlocBuilder<CreateUjianBloc, CreateUjianState>(
                        builder: (context, state) {
                          return state.maybeWhen(
                            orElse: () {
                              return const Center(
